...
impl<'psbt, Pk: MiniscriptKey + ToPublicKey> Satisfier<Pk> for PsbtInputSatisfier<'psbt> {
    ...
    fn lookup_hash160(&self, h: &Pk::Hash160) -> Option<Preimage32> {
        self.psbt.inputs()[self.index]
            .hash160_preimages
            .get(&ElementsHash160::from_byte_array(
                Pk::to_hash160(h).to_byte_array(),
            ))
    ...
    fn lookup_sha256(&self, h: &Pk::Sha256) -> Option<Preimage32> {
        self.psbt.inputs()[self.index]
            .sha256_preimages
            .get(&ElementsSha256::from_byte_array(
    ...
    fn lookup_hash256(&self, h: &Pk::Hash256) -> Option<Preimage32> {
        self.psbt.inputs()[self.index]
            .hash256_preimages
            .get(&ElementsSha256d::from_byte_array(
    ...
    fn lookup_ripemd160(&self, h: &Pk::Ripemd160) -> Option<Preimage32> {
        self.psbt.inputs()[self.index]
            .ripemd160_preimages
            .get(&ElementsRipemd160::from_byte_array(
                Pk::to_ripemd160(h).to_byte_array(),
            ))
            .and_then(try_vec_as_preimage32)
    }
}
...
mod tests {
    ...
    use elements::{
        confidential, AssetId, AssetIssuance, BlockHash, LockTime, OutPoint, TxIn, TxInWitness,
        TxOut,
    };
    ...
    fn test_extract_psbt() {
        ...
        let dummy_hash = BlockHash::GENESIS_PREVIOUS_BLOCK_HASH;
    ...
    fn test_update_input_checks() {
        ...
        let mut non_witness_utxo = elements::Transaction {
            ...
            output: vec![TxOut {
                ...
                script_pubkey: Script::from_hex_no_prefix(
